...
**静•电** 是一款为 macOS 量身打造的轻量能耗洞察工具。它不做复杂面板、不打扰工作流，只在关键时刻给出明确答案：**电池还剩多久、系统在耗什么、谁在偷电**。

通过菜单栏的一屏信息，把电量曲线、应用能耗、智能口径融合在一起，适合通勤办公、移动开发与高强度创作场景。

## ✨ 核心亮点

### 1) 电池状态一目了然
- **实时仪表**：电量、功率 (W)、电流 (mA)、电压 (V)、温度即时呈现
- **48 小时曲线**：平滑贝塞尔曲线 + 充电段高亮，掉电节奏一眼可读
...
- **实时排行**：秒级刷新当前高 CPU 应用
- **36 小时历史**：完整记录应用能耗轨迹
- **能耗贡献曲线**：不是 CPU 占比，而是对总耗电的贡献比例
- **休眠断点识别**：自动标注休眠区间，避免误读
...
- **菜单内直观切换**：无需关闭菜单即可刷新排行

### 4) 数据永不丢
- **自动保存**：退出或重启后历史数据自动恢复
- **5 分钟备份**：后台定时备份，安全可靠
- **报告导出**：CSV / JSON 一键导出，便于分析与对比
